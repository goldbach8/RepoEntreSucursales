                        ...
                        if len(df_proc) == 0:
                            ...
                            if modo_analisis == "Reposición (Envío)":
                                ...
                                for suc in ['ba', 'mdz', 'slt']:
                                    ...
                                    # Pandas maneja inf, pero para visualización es mejor.
                                ...
                                for i, suc in enumerate(sucursales_view):
                                    with tabs[i]:
                                        ...
                                        # Botones de Descarga Final Centrados
                                        st.divider()
                                        ...
                                        with c_down1:
                                            ...
                                            st.download_button(
                                                ...
                                                data=csv_resumen, 
                                                file_name="resumen_reposicion.csv", 
                                                mime="text/csv", 
                                                type="secondary",
                                                use_container_width=True,
                                                key=f"download_resumen_{suc}"
                                            )

                                        # Descarga Completa
                                        with c_down2:
                                            csv_res = df_display_final.to_csv(index=False).encode('utf-8')
                                            st.download_button(
                                                label="💾 DESCARGAR COMPLETA (.CSV)", 
                                                data=csv_res, 
                                                file_name="resultado_reposicion.csv", 
                                                mime="text/csv", 
                                                type="primary",
                                                use_container_width=True,
                                                key=f"download_repo_{suc}"
                                            )
                            ...
                            # ----------------------------------------------------
                                ...
                                # 2. Lógica de Excedentes (Usando el parámetro de entrada)
                                ...
                                # Descarga Global de Devoluciones
                                st.divider()
                                csv_dev = df_dev.to_csv(index=False).encode('utf-8')
                                st.download_button(
                                    "💾 Descargar Reporte de Excedentes (CSV)",
                                    csv_dev,
                                    "analisis_devoluciones.csv",
                                    "text/csv",
                                    type="primary"
                                )
