...
DB_CONFIG = {
    'DRIVER': 'ODBC Driver 17 for SQL Server', # O 'SQL Server' dependiendo de tu instalación
    'SERVER': '10.7.0.36',       # Ej: localhost\SQLEXPRESS o IP
    'DATABASE': 'svsf',
    'UID': 'marianodeiriondo',           # Dejar vacío si usas autenticación de Windows
    'PWD': 'njpXGb6nEXjUUgFsrn'           # Dejar vacío si usas autenticación de Windows
}
...
def obtener_datos_sql():
    ...
    print("🚀 Ejecutando consulta SQL (puede tardar unos minutos)...")
    
    try:
        df = pd.read_sql(query, engine)
        print(f"✅ Consulta exitosa. Filas obtenidas: {len(df)}")
        
        # --- LIMPIEZA Y FORMATO PARA EL SISTEMA ---
        # Llenar nulos con 0 en columnas numéricas si vienen como NaN
        numeric_cols = [col for col in df.columns if 'QPres' in col or 'QRem' in col or 'Stock' in col]
        df[numeric_cols] = df[numeric_cols].fillna(0)
        
        # Guardar en CSV compatible (UTF-8, separado por comas)
        nombre_archivo = 'bajada_repo_tactica.csv'
        df.to_csv(nombre_archivo, index=False, encoding='utf-8', sep=',')
        
        print(f"💾 Archivo guardado correctamente como: {nombre_archivo}")
        print("➡️  Ya puedes cargar este archivo en la aplicación de Streamlit.")
